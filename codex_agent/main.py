...
def verify_codex_cli(timeout_seconds: int = 10) -> bool:
    """Return True if the Codex CLI can be executed inside this container."""
    for cmd in CODEX_COMMANDS:
        try:
            ...
            print(f"[CODEX CHECK] {' '.join(cmd)} -> {output}")
            return True
        except FileNotFoundError:
            # Command is not installed in the image.
            continue
        except subprocess.CalledProcessError as exc:
            error_output = (exc.stderr or exc.stdout or "").strip()
            print(f"[CODEX CHECK] {' '.join(cmd)} failed: {error_output}")
        except subprocess.TimeoutExpired:
            print(f"[CODEX CHECK] {' '.join(cmd)} timed out after {timeout_seconds}s")
    return False


def codex_login_with_api_key(timeout_seconds: int = 30) -> bool:
    """
    Log Codex CLI in using OPENAI_API_KEY from the environment via --with-api-key.
    Returns True on success, False otherwise.
    """
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        print("[CODEX LOGIN] OPENAI_API_KEY is not set; cannot log in Codex CLI.")
        return False

    try:
        print("[CODEX LOGIN] Logging Codex CLI in with API key from environment...")
        result = subprocess.run(
        ...
        print("[CODEX LOGIN] Login stdout:")
        print(result.stdout)
        if result.stderr:
            print("[CODEX LOGIN] Login stderr:")
            print(result.stderr)
        return True
    except subprocess.CalledProcessError as exc:
        print(f"[CODEX LOGIN] Login failed with exit code {exc.returncode}")
        print(exc.stdout or "")
        print(exc.stderr or "")
        return False
    except subprocess.TimeoutExpired:
        print(f"[CODEX LOGIN] Login timed out after {timeout_seconds}s")
        return False
...
def download_request_files(request_id: str, dest_dir: str) -> List[str]:
    """
    Download the template and PDF files for a given request into dest_dir.
    Returns a list of local filenames that were written.
    """
    print(f"[DOWNLOAD] Preparing to download files for request: {request_id}")
    ...
    print(
        f"[DOWNLOAD] Request found: {request_data.get('template_filename')}, "
        f"{request_data.get('pdf_count')} PDFs"
    )
    ...
    if template_blob_path:
        ...
        template_local_name = f"template.{template_ext}"
        template_local_path = os.path.join(dest_dir, template_local_name)
        template_blob.download_to_filename(template_local_path)
        print(f"[DOWNLOAD] Downloaded template: {template_blob_path} -> {template_local_path}")
        downloaded_files.append(template_local_name)
    ...
    for pdf_blob_path in pdf_blob_paths:
        ...
        if not pdf_blob.exists():
            print(f"[DOWNLOAD] Warning: PDF file not found in storage: {pdf_blob_path}, skipping")
            continue

        local_name = os.path.basename(pdf_blob_path)
        local_path = os.path.join(dest_dir, local_name)
        pdf_blob.download_to_filename(local_path)
        print(f"[DOWNLOAD] Downloaded PDF: {pdf_blob_path} -> {local_path}")
        downloaded_files.append(local_name)

    print(f"[DOWNLOAD] Total files downloaded: {len(downloaded_files)}")
...
if __name__ == "__main__":
    ...
    if not codex_login_with_api_key():
        print("[MAIN] Codex login failed or OPENAI_API_KEY not set. Continuing may still fail.")

    # 2) Verify CLI is present.
    if not verify_codex_cli():
